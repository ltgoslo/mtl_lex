...
class Lexicon_Embedding(nn.Module):
    ...
    def __init__(self, word2idx,
                 ...
                 embedding_dim,
                 dropout=0.3):
        super(Lexicon_Embedding, self).__init__()
...
class Concat_Embed_Model(nn.Module):

    def __init__(self, word2idx,
                 embedding_matrix,
                 lexicon_embedding_network,
        ...
        super(Concat_Embed_Model, self).__init__()
        self.embedding_dim = embedding_dim
        self.lexicon_embedding_network = lexicon_embedding_network
        ...
        # LSTM input is embedding dim x 2, as we will concatenate
        # the lexicon embeddings to the pretrained word embeddings
    ...
    def max_pool(self, x):
        ...
        lexicon_emb = self.lexicon_embedding_network.word_embeds(x.data)
        concatenated = torch.cat((emb, lexicon_emb), dim=1)

        dropped = self.word_dropout(concatenated)
...
if __name__ == "__main__":
    ...
    model = Lexicon_Embedding(vocab, new_matrix, 2, 300)
    ...
    print("Training lexicon embedding")
    ...
    optimizer = optim.Adam(sentiment_model.parameters())
    ...
    for epoch in range(10):
        ...
        for x, y in maintask_loader:
            loss = sentiment_model.pooled_sentiment_loss(x, y)
            full_loss += loss
            loss.backward()
            optimizer.step()
        print("Loss: {0:.3f}".format(full_loss / len(maintask_loader)))

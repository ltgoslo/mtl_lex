...
from lexicon_embedding_model import *
...
def train_model(vocab,
    ...
    basedir = os.path.join("concat_models",
                           args.DATASET,
                           args.FINE_GRAINED,
                           args.AUXILIARY_TASK,
                           args.LEXICON)
    ...
    for i, run in enumerate(range(number_of_runs)):
        ...
        lexicon_embedding_model = Lexicon_Embedding(vocab,
                                                    new_matrix,
                                                    2,
                                                    embedding_dim)

        # Set our optimizers
        auxiliary_optimizer = torch.optim.Adam(lexicon_embedding_model.parameters(), lr=auxiliary_learning_rate)
        ...
        for j, epoch in enumerate(range(epochs)):
            ...
            for batch in tqdm(range(num_lex_batches)):
                ...
                x = torch.tensor(x)
                y = torch.tensor(y)
                lexicon_embedding_model.zero_grad()
                loss = lexicon_embedding_model.projection_loss(x, y)
                full_loss += loss
                loss.backward()
                auxiliary_optimizer.step()

            lexicon_embedding_model.eval()
            print("Loss: {0:.3f}".format(full_loss / num_lex_batches))
            print("Train acc")
            x, y = zip(*lex_train)
            x = torch.tensor(x)
            y = torch.tensor(y)
            acc = lexicon_embedding_model.eval_aux(x, y)
            print()
            ...
            batch_losses = 0
            num_batches = 0
            lexicon_embedding_model.train()

        for j, epoch in enumerate(range(epochs)):
            print("epoch {0}: Training main task...".format(epoch + 1))
            sentiment_model = Concat_Embed_Model(vocab,
                                                 new_matrix,
                                                 lexicon_embedding_model,
                                                 num_labels,
            ...
            sentiment_optimizer = torch.optim.Adam(sentiment_model.parameters(), lr=sentiment_learning_rate)

            sentiment_model.train()
            for sents, targets in maintask_loader:
                sentiment_model.zero_grad()

                loss = sentiment_model.pooled_sentiment_loss(sents, targets)
            ...
            f1, acc, preds, ys = sentiment_model.eval_sent(maintask_train_iter,
                                                 batch_size=BATCH_SIZE)
            f1, acc, preds, ys = sentiment_model.eval_sent(maintask_dev_iter,
                                                 batch_size=BATCH_SIZE)
            ...
            if acc > best_dev_acc:
                ...
                basedir = os.path.join("concat_models",
                                       ...
                                       args.LEXICON,
                                       "{0}".format(run + 1))
                outname = "epochs:{0}-lstm_dim:{1}-lstm_layers:{2}-devacc:{3:.3f}".format(epoch + 1, sentiment_model.lstm.hidden_size, sentiment_model.lstm.num_layers, acc)
                modelfile = os.path.join(basedir,
                                         outname)
                os.makedirs(basedir, exist_ok=True)
                print("saving model to {0}".format(modelfile))
                torch.save(sentiment_model.state_dict(), modelfile)
